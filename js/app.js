...
function setupSortAndFilterListeners() {
    ...
    if (sortSelect) {
        sortSelect.addEventListener('change', () => {
            ...
            renderResultsWithSortAndFilter();
        });
    }
    if (countrySelect) {
...
// 渲染结果时应用排序和国家筛选
function renderResultsWithSortAndFilter() {
    let results = [...lastResultsRaw];
    // 国家筛选
    if (currentCountry) {
        results = results.filter(item => (item.vod_area || '').includes(currentCountry));
    }
    // 只保留有年份的
    results = results.filter(item => extractYear(item) > 0);
    // 终极排序
    results.sort((a, b) => {
        const yearA = extractYear(a);
        const yearB = extractYear(b);
        if (yearA !== yearB) return currentSortOrder === 'desc' ? yearB - yearA : yearA - yearB;
        // 年份相同再按名称
...
function renderResults(results) {
    ...
    if (!results || results.length === 0) {
        resultsDiv.innerHTML = `<div class="col-span-full text-center py-16">没有找到匹配的结果</div>`;
            return;
        }
...
// 动态生成国家选项
function updateCountryOptions(results) {
    const countrySelect = document.getElementById('countrySelect');
    if (!countrySelect) return;
    const countrySet = new Set();
    results.forEach(item => {
        if (item.vod_area) {
            item.vod_area.split(/[,，\s]/).forEach(area => {
                if (area && area.length <= 8) countrySet.add(area.trim());
            });
        }
    });
    countrySelect.innerHTML = '<option value="">全部</option>' +
        Array.from(countrySet).sort().map(c => `<option value="${c}">${c}</option>`).join('');
}

// 智能提取年份（优先 vod_year，其次 vod_remarks，再次 vod_name）
function extractYear(item) {
    if (item.vod_year && /^\d{4}$/.test(item.vod_year)) return parseInt(item.vod_year, 10);
    if (item.vod_remarks) {
        const match = item.vod_remarks.match(/(19|20)\d{2}/);
        if (match) return parseInt(match[0], 10);
    }
    if (item.vod_name) {
        const match = item.vod_name.match(/(19|20)\d{2}/);
        if (match) return parseInt(match[0], 10);
    }
    return 0;
}
...
async function search() {
    ...
    try {
        ...
        // --- [START] This entire block replaces the old result handling logic ---

        // 1. First, filter content if the filter is enabled.
        ...
        // 2. Save the final, clean, raw results for later use by filters.
        lastResultsRaw = allResults;

        // --- [Definitive Fix Start] ---

        // 1. Update UI elements and set default state
        updateCountryOptions(lastResultsRaw);
        // 2. 设置默认排序和筛选状态
        ...
        // 4. Handle other UI updates like history and visibility.
        ...
        try {
            ...
            window.history.pushState({ search: query }, `搜索: ${query} - TTC TV`, `/s=${encodedQuery}`);
            document.title = `搜索: ${query} - TTC TV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
        }
        
        // --- [Definitive Fix End] ---
        // The function `renderResultsWithSortAndFilter` will now only be used by the dropdowns.
